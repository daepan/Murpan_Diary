...
React로 개발하면서 위에 제시된 최적화가 필요한 상황을 맞이하였을 때, 생각할 수 있는 것은 여러가지가 있을 수 있습니다. Throttle, debounce 같은 처리방식도 있고, 위와 같이 렌더링에서는 메모이제이션이라는 방식을 지원하고 이를 React는 훅이나 .memo 와 같은 함수로 지원하고 있습니다.
